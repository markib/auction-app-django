...
urlpatterns = [
    ...
    re_path(
        r"^profile/(?P<user_id>[0-9]+)/$", views.ProfileView.as_view(), name="profile"
    ),
    re_path(
        r"^profile/(?P<user_id>[0-9]+)/edit/$",
    ...
    path(
        "bids_currently_winning/",
        views.BidsCurrentlyWinning.as_view(),
        name="bids_currently_winning",
    ),
    path("bids_won/", views.BidsWon.as_view(), name="bids_won"),
    re_path(r"^ajax/search/$", views.search, name="search"),
    re_path(r"^ajax/options/$", views.options, name="options"),
